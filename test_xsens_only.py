...
def run_diagnostic():
    ...
    plt.plot(df['time_offset'], df['acc_x'], label='Acc X (長邊)', color='red', alpha=0.8)
    plt.plot(df['time_offset'], df['acc_y'], label='Acc Y (短邊)', color='green', alpha=0.8)
    plt.plot(df['time_offset'], df['acc_z'], label='Acc Z (垂直)', color='blue', alpha=0.8)
    
    # 標題與標籤
    plt.suptitle("國立虎尾科技大學 資訊管理系\n先進運動表現實驗室", fontsize=14, fontweight='bold')
    plt.title(f"Xsens 加速度計校準測試 - 裝置: {dev_id}", fontsize=12, pad=20)
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.92]) # 調整間距避免標題重疊
